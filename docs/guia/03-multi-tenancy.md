...
`findUnique`, `findUniqueOrThrow`, `update`, `delete` e `upsert` **não estão em `WHERE_OPS`**.
Chamando qualquer uma delas num model de `TENANT_MODELS`, nenhum dos `if`/`else if` da função
bate, e a query segue pro banco exatamente como foi escrita — **sem nenhum filtro de
organização**.
...
O script cria duas organizações reais no Postgres local, popula cada uma com dado próprio
(membro, empresa, conexão de integração, campanha, job de extração, conversa) e roda **9
asserções** de que uma organização não enxerga nem afeta dado da outra — inclusive um
`updateMany` com filtro de organização trocado, que precisa afetar zero linhas. Se você
mudar algo em `tenantDb`, na DAL ou nas actions e esse script continuar verde, o isolamento
continua de pé; se ele falhar, alguma mudança furou a fronteira.

## Resumo

- Tabela de negócio (está em `TENANT_MODELS`?) → sempre via `tenantDb(orgId)`, nunca
  `prisma` cru.
- Ler por id: `findFirst({ where: { id } })`. Nunca `findUnique`.
...
- `upsert` em tabela de negócio: sem atalho — some `organizationId` ao `where` à mão.

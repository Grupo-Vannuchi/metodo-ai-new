...
/**
 * Upload a file attachment to a task. Bytes go straight to object storage
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const task = await db.task.findFirst({ where: { id }, select: { id: true } });
  if (!task) return Response.json({ ok: false, error: "not_found" }, { status: 404 });

  const count = await db.taskAttachment.count({ where: { taskId: id } });
  ...
  const key = `tasks/${id}/${randomUUID()}-${safeName}`;

...
const MAX_COUNT = 20; // per opportunity
...
/**
 * Upload a file attachment to an opportunity. Bytes go straight to object
 * storage (Supabase/local) — never Postgres; only the metadata + URL is stored.
 */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const opp = await db.opportunity.findFirst({ where: { id }, select: { id: true } });
  if (!opp) return Response.json({ ok: false, error: "not_found" }, { status: 404 });

  const count = await db.opportunityAttachment.count({ where: { opportunityId: id } });
  ...
  const key = `opportunities/${id}/${randomUUID()}-${safeName}`;

  try {
    const stored = await putMedia(key, buffer, mime);
    const attachment = await db.opportunityAttachment.create({
      data: {

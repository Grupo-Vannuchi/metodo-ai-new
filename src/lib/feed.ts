/** Quick-reaction emoji set for the team feed (client + server). Reactions are
 * validated against this allow-list so arbitrary input can't be stored. */
export const FEED_EMOJIS = ["👍", "❤️", "😂", "🎉", "😮", "👏"] as const;
export type FeedEmoji = (typeof FEED_EMOJIS)[number];

/** Ephemeral posts live for 24h; permanent/pinned posts have no expiry. */
export const FEED_TTL_MS = 24 * 60 * 60 * 1000;

/** Post categories, in display order (mirrors the FeedCategory enum). */
export const FEED_CATEGORIES = ["GENERAL", "ANNOUNCEMENT", "NEWS", "EVENT", "PRAISE"] as const;
export type FeedCategoryKey = (typeof FEED_CATEGORIES)[number];
